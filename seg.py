...
import models
from utils_demo import torch_to_list, to_torch, im_to_numpy, im_to_torch, resize_generic, color_normalize, save_pred
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--video_path",
        type=Path,
        default="sample_data/demo_video.mp4",
    ...
    pred = main_mstcn(features, logits, **vars(p.parse_args()))

    #use the prediction vector to output video segments
    import skvideo.io  
    #videodata = skvideo.io.vread("sample_data/demo_video.mp4")
    videodata = skvideo.io.vread(str(vars(args)['video_path']))  
    print(videodata.shape)

    pred_pad = [0]*8 + pred + [0]*7 #pad the prediction to match the leangth of the video


    lastOneIdx = len(pred_pad) - 1 - pred_pad[::-1].index(1)
    firstOne = pred_pad.index(1)
    sp = pred_pad.index(0,firstOne) #start pointer
    ep = sp #end pointer
    outIdx = []
    ep = pred_pad.index(1,sp)
    while sp < lastOneIdx:#last stream of zeros
        if ep - sp > 8:
            outIdx.append([sp,ep]) #add 4 additional frames to enhance the accuracy of segment
            sp = pred_pad.index(0,ep)
            try:
                  ep = pred_pad.index(1,sp)
            except:
                  print("end of list")
            ...
            ep = pred_pad.index(0,ep)
            try:
                  ep = pred_pad.index(1,ep)
            except:
                  print("end of list")
    ...
    for elem in outIdx:
        ...
        ep = elem[1]
        if ep - sp < 35:
            outIdxCleared.append(elem)
    outIdxCleared



    #clear previous segments
    files = glob.glob('outSegments/*')
    for f in files:
        os.remove(f)

    #saving segments
    idx = 1
    for elm in outIdxCleared:
        skvideo.io.vwrite("outSegments/seg"+str(idx)+".mp4",videodata[elm[0]:elm[1],:,:,:])
        idx +=1
